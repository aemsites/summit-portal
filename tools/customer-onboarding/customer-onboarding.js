...
// DA sheets are stored as HTML at the path without extension.
// Never write to the .json path — that is generated by AEM on preview/publish.
...
// Parse the DA HTML source of a sheet into an array of { header: value } objects.
...
// Serialize headers + rows back to a minimal HTML table DA can read.
function buildSheetHtml(headers, rows) {
  const th = headers.map((h) => `<th>${h}</th>`).join('');
  const tbody = rows.map((row) => {
    const cells = headers.map((h) => `<td>${row[h] ?? ''}</td>`).join('');
    return `<tr>${cells}</tr>`;
  }).join('\n');
  return `<html><body><table><tr>${th}</tr>\n${tbody}\n</table></body></html>`;
}
