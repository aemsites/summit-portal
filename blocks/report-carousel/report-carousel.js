...
export default function init(el) {
  ...
  if (downloadLink) {
    ...
    if (isReportDisabled) {
      dlBtn.href = '#';
      dlBtn.classList.add('rc-download-disabled');
      dlBtn.setAttribute('aria-disabled', 'true');
      dlBtn.setAttribute('tabindex', '-1');
      dlBtn.setAttribute('title', disabledMessage);
    ...
    if (isReportDisabled) {
      ...
      dlGroup.className = 'rc-download-group';
      const dlNote = document.createElement('span');
      dlNote.className = 'rc-download-note';
      dlNote.textContent = disabledMessage;
      dlGroup.append(dlNote, dlBtn);
      tabBar.append(dlGroup);
    } else {
      tabBar.append(dlBtn);
    }
  }
  ...
  if (downloadLink) {
    ...
    if (isReportDisabled) {
      dlBtnFooter.href = '#';
      dlBtnFooter.classList.add('rc-download-disabled');
      dlBtnFooter.setAttribute('aria-disabled', 'true');
      dlBtnFooter.setAttribute('tabindex', '-1');
      dlBtnFooter.setAttribute('title', disabledMessage);
      dlBtnFooter.addEventListener('click', (e) => e.preventDefault());
    }
    ...
    if (isReportDisabled) {
      const dlGroupFooter = document.createElement('div');
      dlGroupFooter.className = 'rc-download-group';
      const dlNoteFooter = document.createElement('span');
      dlNoteFooter.className = 'rc-download-note';
      dlNoteFooter.textContent = disabledMessage;
      dlGroupFooter.append(dlNoteFooter, dlBtnFooter);
      slideFooter.append(dlGroupFooter);
    } else {
