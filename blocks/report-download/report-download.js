...
export default function init(el) {
  ...
  const state = getReportDownloadState();
  ...
  if (isDisabled) {
    ...
    unavailableMsg.className = 'rd-unavailable-msg';
    unavailableMsg.textContent = UNAVAILABLE_MESSAGE;
    ctaRow.append(unavailableMsg);
  ...
  tagEl.href = isDisabled ? '#' : downloadHref;
  if (isDisabled) {
    tagEl.setAttribute('aria-disabled', 'true');
    tagEl.setAttribute('tabindex', '-1');
    tagEl.addEventListener('click', (e) => e.preventDefault());
  }
  ...
  if (isDisabled) {
    card.classList.add('rd-pdf-card-disabled');
    right.classList.add('rd-right-disabled');
  }
