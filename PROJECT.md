...
### docs
Zero-output theme switch for documentation pages. `init()` adds a `docs-page` class to `<body>` then removes the block (and its now-empty wrapper/section). Its auto-loaded `docs.css` scopes **every** rule under `.docs-page`, so dropping the block onto a page restyles only that page — a clean reading layout: a centered ~940px column, left-aligned dark section headings with divider rules, a framed hero card, refined `report-cards` (2-up grid; `steps` rendered as a numbered vertical list), readable `table`s with zebra rows, and `report-callout` bars with a red left accent. Light/dark aware via `light-dark()`; edge-to-edge with a 20px text gutter below 1000px. Authoring: place an empty `docs` block first on the page.
